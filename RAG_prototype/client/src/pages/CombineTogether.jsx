...
const CombineTogether = () => {
  ...
  const [shouldCallRAG, setShouldCallRAG] = useState(false);
  ...
  const handleRAG = useCallback(async () => {
    ...
    const data = {
      prompt: (`You are a senior proofreader to proofread and review translation from ${sourceLang} to ${targetLang}.
                The source text is [${prompt}].
                The translation to be proofread is [${translatedText}].
                Additionally, please consider that the translation is in the context of financial reporting. The translation should have a formal and precise style and tone. The translation should be concise and maintain important information. Please use the context of the files.
                Your response should include only the text of the revised version of the translation.
                `),
    };
  ...
  useEffect(() => {
    if (shouldCallRAG) {
      handleRAG();
      setShouldCallRAG(false);
    }
  }, [translatedText, shouldCallRAG, handleRAG]);
    ...
    <div>
      ...
      <div className="translation-app">
        ...
        <div className="button-container">
          ...
          <button onClick={handleRAG}>RAG Generate</button>
        </div>
      </div>
    </div>
  );
...
export default CombineTogether;